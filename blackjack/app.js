...
function computeScore(cards) {
  ...
  // ...
  // return cards.reduce((a,b) => (a.value+b.value));

  var total = 0;

  return cards.forEach(function(a){
    if (a.value === "KING"){
      a.value = 10;
      total += a.value;
    }
...
function newHand() {
  ...
  */
  ...
  .then(response.response.json())
  .then(function(data){
    playerCards.push(data[0].value, data[1].value);
    dealerCards.push(data[2].value, data[3].value);
